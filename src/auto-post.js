...
文体の条件:
- 冒頭は「<主体>が<何に役立ち/何が簡単にできて>」の一言で始め、以下の語を自然に用いて良い: 「試す価値ある」「価値あり」「有益すぎる」。不自然な「価値がある」は使わない（非エンジニア語彙、常体、感嘆符は1回まで）
- 次の文で実用価値を具体的に2点示す（何がどの場面で助かるか）
- 「これを〜しておけば、〜から〜できる」の型を1回含める
- スター数や人気指標などの数値は書かない。絵文字・ハッシュタグ・URL・記号#は入れない
- ドキュメント/README を読めばわかる等のメタ的な言及は書かない
- 締めは導入の一歩を短く触れるが、具体的なファイル名（README等）は避ける（例: 数分で試せる手順が用意されている）
- 2〜3文、**約130文字以内**。常体（です・ます調は避ける）
