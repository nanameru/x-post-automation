...
文体の条件:
- 冒頭は「<主体>が<何ができて/何に役立って>面白い。」の形で、非エンジニアにも伝わる言葉で簡潔に書く
- 2〜3文、最大260文字。絵文字・ハッシュタグなし。常体（です・ます調は避ける）
- 「これを〜しておけば、〜から〜できる」の型を1回含める（例: これをブックマークしておけば、必要な場面から最短で使える）
- 誇張表現を避け、具体的な価値を2点ほど挙げる（例: 何がどの場面で助かるか）
- スター数や過度な数値は原則入れない
- URLは本文に入れない（本文の直後に改行し、コード側でURLを1行付ける）
- 締めは「導入の一歩」を一言で触れる（例: README手順で数分で試せる）
