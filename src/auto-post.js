...
class GitHubTrendingBot {
  ...
  async generateTweetText(repoDetails, trendingInfo) {
    const prompt = `
あなたは短く鋭い日本語のテック投稿ライターです。次の情報から、誰でも理解しやすい実用重視のポスト文を1つだけ作ってください。

文体の条件:
- 冒頭は「<主体>が<何に役立ち/何が簡単にできて>試す価値ある/価値あり」と一読で伝わる一言（非エンジニア語彙、常体、感嘆符は1回まで）。「価値がある」の表現は使わない
- 次の文で実用価値を具体的に2点示す（何がどの場面で助かるか）
- 「これを〜しておけば、〜から〜できる」の型を1回含める
- スター数や人気指標などの数値は書かない。絵文字・ハッシュタグ・URL・記号#は入れない
- ドキュメント/README を読めばわかる等のメタ的な言及は書かない（例: READMEの手順で始められる、公式手順で〜）
- 締めは導入の一歩を短く触れるが、具体的なファイル名（README等）は避ける（例: 数分で試せる手順が用意されている）
- 2〜3文、最大260文字。常体（です・ます調は避ける）
...
出力: 本文のみ（1つ）。先頭/末尾の空白なし。`;
    ...
    console.log('🟡 OpenAI returned empty content after retries. Falling back to repo name/description.');
